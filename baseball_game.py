...
def is_validated_number(user_input_number):
    ...
    if is_digit(user_input_number) == True and is_between_100_and_999(user_input_number) == True and is_duplicated_number(user_input_number) == True:
        return True
    else:
        return False
...
def main():
    ...
    while True:
        ...
        while get_strikes_or_ball(str(user_input),str(random_number)) != [3,0]:
            ...
            check = get_strikes_or_ball(str(user_input),str(random_number))
            if check == [3,0]:
                print("Strikes : 3, Balls : 0")

                while True:
                    checkAgain = input("You win, one more(Y/N)?")
                    if is_yes(checkAgain) == True:
                        break
                    elif is_no(checkAgain) == True:
                        break
                    else:
                        print("Wrong Input")
            else:
                print("Strikes : %s, Balls : %s"%(check[0],check[1]))

        if user_input == '0':
            break

        if is_no(checkAgain) == True:
            break
